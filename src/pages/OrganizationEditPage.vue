<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Edit Organization</h1>
      </v-col>
    </v-row>
        ...
        <v-card>
          <v-card-text>
            <AutoSaveField
              ...
              automation-id="organization-edit-name-input"
            />

            <AutoSaveField
              :model-value="organization.description || ''"
              label="Description"
              ...
              textarea
              :rows="3"
              automation-id="organization-edit-description-input"
            />

            <AutoSaveSelect
              :model-value="organization.status || 'active'"
              label="Status"
              :items="statusOptions"
              :on-save="(value: string) => updateField('status', value)"
              class="mt-4"
              automation-id="organization-edit-status-select"
            />
            ...
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(organization.created.at_time)"
                  label="Created"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="organization.created.by_user"
                  label="Created By"
                  readonly
                  variant="outlined"
                  density="compact"
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(organization.saved.at_time)"
                  label="Last Saved"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="organization.saved.by_user"
                  label="Last Saved By"
                  readonly
                  variant="outlined"
                  density="compact"
            ...
            <v-card-actions class="px-0 mt-4">
              <v-btn 
                ...
                variant="text"
                data-automation-id="organization-edit-back-button"
              >
                Back to List
              </v-btn>
            </v-card-actions>
...
/**
 * Organization Edit Page - Showcase of spa_utils AutoSave components
 * 
...
const { data: organization, isLoading, error: queryError } = useQuery({
  queryKey: ['organization', organizationId],
  queryFn: () => api.getOrganization(organizationId.value),
})
...
const { mutateAsync: updateOrganization } = useMutation({
  mutationFn: (data: OrganizationUpdate) => api.updateOrganization(organizationId.value, data),
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ['organization', organizationId.value] })
...
async function updateField(field: keyof OrganizationUpdate, value: string) {
  try {
